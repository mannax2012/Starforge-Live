...
WeaponsLightsaberVendorLogic = ScreenPlay:new {
	scriptName = "WeaponsLightsaberVendorLogic",
	currencies = {
			{currency = "credits"},
...
function WeaponsLightsaberVendorLogic:openSUILightsaberGenThree(pCreatureObject, pUsingObject)
	...
	sui.setTitle("Lightsaber Gen3 Schematics")
...
function WeaponsLightsaberVendorLogic:getMerchandiseStringLightsaberGenThree(num)
...
function WeaponsLightsaberVendorLogic:hasEnoughCurrencyLightsaberGenThree(pPlayer, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function WeaponsLightsaberVendorLogic:payTokensLightsaberGenThree(pPlayer, selectedItem, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
--GEN4
function WeaponsLightsaberVendorLogic:openSUILightsaberGenFour(pCreatureObject, pUsingObject)
	...
	sui.setTitle("Lightsaber Gen4 Schematics")

	local message = "Please select which item you want to buy."
	sui.setPrompt(message)

	for i = 1, #self.merchandise_weapon_lightsaber_gen4, 1 do
...
function WeaponsLightsaberVendorLogic:buyItemLightsaberGenFour(pPlayer, itemSelected)
	local merch = self.merchandise_weapon_lightsaber_gen4[itemSelected]
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function WeaponsLightsaberVendorLogic:getMerchandiseStringLightsaberGenFour(num)
	local merch = self.merchandise_weapon_lightsaber_gen4[num]
	local merchString = merch.name .. " ("
...
function WeaponsLightsaberVendorLogic:hasEnoughCurrencyLightsaberGenFour(pPlayer, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
...
function WeaponsLightsaberVendorLogic:payTokensLightsaberGenFour(pPlayer, selectedItem, num)
	local pInventory = CreatureObject(pPlayer):getSlottedObject("inventory")
	local containerSize = SceneObject(pInventory):getContainerObjectsSize()
	local pGhost = CreatureObject(pPlayer):getPlayerObject()
	
	local merch = self.merchandise_weapon_lightsaber_gen4[selectedItem]
	local tokenCost = merch.cost[num]
	deleteItems = 0
	tokens = 0
	screenPlayTokenCost = 0
